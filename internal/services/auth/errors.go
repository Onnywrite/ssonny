...
func userFailed(log *zerolog.Logger, err error) error {
	...
	case errors.Is(err, repo.ErrChecked):
		log.Debug().Err(err).Msg("some data does not satisfy repo, maybe validation is not up to date with database schema")
		return erix.Wrap(err, erix.CodeBadRequest, ErrInvalidData)
	case errors.Is(err, repo.ErrFK):
		log.Debug().Err(err).Msg("FK constraint violation, either row with this PK does not exist or you forgot 'delete cascade'")
